...
extension Double {
    ...
    /// ```
    private var currenyFormatter2: NumberFormatter {
    ...
    func asCurrencyWith2Decimals() -> String {
        ...
        return currenyFormatter6.string(from: number) ?? "$0.00"
    }
    ...
    /// ```
    private var currenyFormatter6: NumberFormatter {
        let formatter = NumberFormatter()
        formatter.usesGroupingSeparator = true
        formatter.numberStyle = .currency
        //formatter.locale = .current
    ...
    /// Converts a Double to a String representation with percent symbol
    /// ```
    /// Convert 1.2345 to "1.23%"
    /// ```
